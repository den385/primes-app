...
namespace BPSW
{
  void ReportProgress( double percent )
	{
		Wrapper::hashtableProgressCallback( (System::Double)percent );
	}

	//! Модуль 64-битного числа
	 long long Wrapper::abs (long long n)
	{
		return n < 0 ? -n : n;
	}

	 unsigned long long Wrapper::abs (unsigned long long n)
	{
		return n;
	}
	...
	T2 prime_div_trivial (const T & n, T2 m)
	{
		...
		for (auto iter = primes; iter!=primes + 168 && *iter <= m; ++iter)
		{
			const int div = *iter;
			if (div * div > n)
				break;
	...
	//! Алгоритм Бэйли-Померанс-Селфридж-Вагстафф (BPSW) проверки n на простоту
	template <class T>
	bool Wrapper::isprime (T n)
	{
		return baillie_pomerance_selfridge_wagstaff (n);
	}

	bool Wrapper::IsPrime( System::UInt64 value )
	{
		long long a = (long long) value;

		bool result = isprime<long long>( a );

		return result;
	}

	void Wrapper::InitHashtable( System::UInt64% lower, System::UInt64% upper, bool silent, System::Action<System::Double>^ callback )
	{
		hashtableProgressCallback = callback;
		auto bounds = ::Init( silent, &BPSW::ReportProgress );
		lower = bounds.first;
		upper = bounds.second;		
	}
	
	bool Wrapper::RequestHashtable( System::UInt64 target )
	{
		return ::Request( target );
	}

	void Wrapper::DestroyHashtable()
	{
		::Destroy();
	}
}
